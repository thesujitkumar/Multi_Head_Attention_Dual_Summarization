...
class DocLSTM(nn.Module):
    ...
    def forward(self, body):
        ...
        del rinputs_list
        # print("dimension of headline input using BILSTM ",Bi_Head_h.shape)
        # print("dimension of headline input using BILSTM ",Bi_Head_h)
        ...
        low_sim_conv_vec= torch.flatten(low_sim_pooled)
        ...
        return  low_sim_conv_vec, Bi_Head_h, low_sent_feature
...
class Similarity(nn.Module):
    ...
    def forward(self,  conv, head, low):
        ...
        feature_vec=torch.cat((conv,head,low,low_head_diff,low_head_angle),0)
...
class SimilarityTreeLSTM(nn.Module):
    ...
    def forward(self, body,  feature_vec= None):
        conv, head, low_summary= self.doclstm(body)
        output = self.similarity(conv, head, low_summary)
        return output
