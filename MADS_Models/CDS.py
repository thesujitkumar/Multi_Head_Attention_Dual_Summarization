...
class DocLSTM(nn.Module):
    ...
    def forward(self, body):
        ...
        return   high_sim_conv_vec, low_sim_conv_vec, Bi_Head_h
...
class Similarity(nn.Module):
    def __init__(self, mem_dim, hidden_dim, num_classes,max_num_word_head):
        ...
        self.convolution_projection= nn.Linear(140,(2*mem_dim))


        self.wh = nn.Linear((8 * self.mem_dim+80) , self.hidden_dim)  # for only deep feature.
        self.wp = nn.Linear(self.hidden_dim, 2)

    def forward(self,h_conv,l_conv, head):
        h_conv= h_conv.view(1,70)
        l_conv= l_conv.view(1,70)
        head= head.view(1,2*self.mem_dim)


        conv_angle = torch.mul(h_conv, l_conv)
        conv_diff = torch.abs(torch.add(h_conv, - l_conv))
        conv_angle_diff = torch.cat((conv_angle, conv_diff),1 )
        conv_raw_cat = torch.cat((h_conv,l_conv),1)
        conv_feature=torch.cat((conv_angle_diff,conv_raw_cat),1)

        conv=torch.cat((h_conv,l_conv),0)

        conv=self.convolution_projection(conv.view(1,140))

        low_head_angle = torch.mul(conv, head)
        high_head_angle = torch.mul(conv, head)

        feature_vec=torch.cat((conv_feature, low_head_angle,high_head_angle,head),1)
...
class SimilarityTreeLSTM(nn.Module):
    ...
    def forward(self, body):
        h_conv,l_conv, head= self.doclstm(body)
        output = self.similarity(h_conv,l_conv, head)
        return output
