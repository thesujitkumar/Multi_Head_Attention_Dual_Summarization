...
class DocLSTM(nn.Module):
    def __init__(self, vocab_size, in_dim, mem_dim, sparsity, freeze, max_num_para, max_num_sent, max_num_word, num_atten_head, thresold):
        ...
        #self.multihead_atten = nn.MultiheadAttention(2*mem_dim, num_atten_head,batch_first=True)
    ...
    def forward(self, body):
        ...
        for i in range(len(sent_hidden_list)):
            ...
            if head_sent_sim_vec[i] >=self.thresold:
        ...
        "Convolutions feature construtions"
        ...
        for conv in self.convs:
            x=conv(high_sim_sent_conv.view(1,1,self.max_num_sent,200))
            x = torch.squeeze(x, -1)
        ...
        x_l=[]
        for conv in self.convs:
            x=conv(low_sim_sent_conv.view(1,1,self.max_num_sent,200))
            x = torch.squeeze(x, -1)
            x = F.max_pool1d(x, x.size(2))
            x_l.append(x)
        ...
        return   high_sim_conv_vec, low_sim_conv_vec, Bi_Head_h, low_sent_feature,high_sent_feature
...
class Similarity(nn.Module):
    def __init__(self, mem_dim, hidden_dim, num_classes, max_num_word_head):
        ...
        self.wh = nn.Linear((16 * self.mem_dim+80) , self.hidden_dim)  # for only deep feature.
    ...
    def forward(self,  h_conv,  l_conv, head, low, high):
        ...
        feature_vec=torch.cat((conv_feature, low_head_diff,high_head_diff,low_head_angle,high_head_angle, low,high,head),0)
...
class SimilarityTreeLSTM(nn.Module):
    def __init__(self, vocab_size, in_dim, mem_dim, hidden_dim, sparsity, freeze, num_classes, \
        max_num_para, max_num_sent, max_num_word, num_filter, num_head, thresold):
        super(SimilarityTreeLSTM, self).__init__()
        self.hidden_dim = hidden_dim
        self.doclstm = DocLSTM(vocab_size, in_dim, mem_dim, sparsity, freeze, max_num_para, max_num_sent, max_num_word, num_head, thresold)
        self.similarity = Similarity(mem_dim, hidden_dim, num_classes, max_num_word)
    def forward(self, body):
        high_conv, low_conv, head, low, high= self.doclstm(body)
        output = self.similarity(high_conv, low_conv, head, low, high)
        return output
